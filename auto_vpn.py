...
def get_configs_from_source(url):
    try:
        ...
        if response.status_code == 200:
            ...
            # Если полученный текст закодирован в Base64 — декодируем
            try:
...
def filter_working_configs():
    ...
    # Берём первые 30 уникальных серверов
    unique_configs = list(set(working_configs))[:30]

    # Сохраняем как обычный чистый текст (каждый сервер с новой строки)
    plain_text = "\n".join(unique_configs)
